...
def run_command(cmd, cwd=None, logfile_path=None):
    if cwd is not None:
        print_and_log("$ cd {}".format(cwd), logfile_path)
    print_and_log("$ {}".format(cmd), logfile_path)
    ...
    try:
        ...
        if exit_code != 0:
            raise LoggedException(
                ...
                "went wrong.".format(cmd, exit_code), logfile_path)
    except CalledProcessError as e:
        raise LoggedException(
            "Error running '{}':\n{}\n{}".format(cmd, e.output, str(e)),
            logfile_path)
